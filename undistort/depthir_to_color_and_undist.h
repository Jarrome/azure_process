...
//std::cout << "s" << src_width << " " << dst_width << " " << dst_height  << " " <<k4a_image_get_format(src)<<" "<< k4a_image_get_size(src)<<std::flush;
    ...
    for (int i = 0; i < dst_width * dst_height; i++)
    {
        ...
        if (lut_data[i].x != INVALID && lut_data[i].y != INVALID)
        {
            if (type == INTERPOLATION_NEARESTNEIGHBOR)
            {
//std::cout << i << " " << lut_data[i].y * src_width + lut_data[i].x << std::endl << std::flush;
